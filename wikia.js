...
class Wikia {
	...
	async findWikia(query){
		try{
			...
			logger.log(`[wikia] searching wikia ${query}: ${url}`);
			...
			logger.log(`[wikia] error during wikia lookup: ${query}`,error);
			// throw error;
		}
		return null;
	}
	async search(wikiaQuery,query){
		...
		if(wikia){
			...
			try{
				...
				logger.log(`[wikia] searching ${wikiaQuery}: ${url}`);
				const ret = await axios.get(url);
				if(ret&&ret.data&&ret.data.items){
				...
				logger.log(`[wikia] error during wikia ${wikiaQuery} search: ${query}`,error.response.data.status);
				if(error.response.data.error=='ControllerNotFoundException'){
					...
					logger.log(`[wikia] searching html ${wikiaQuery}: ${url}`);
					const ret = await axios.get(url);
